...
describe("Vault path helpers", () => {
  it("normalizes safe paths", () => {
    expect(normalizeVaultPath("/hvsecrets/credentials/example.com/")).toBe(
      "hvsecrets/credentials/example.com",
    );
    expect(joinVaultPath("secret", "data", "hvsecrets")).toBe(
      "secret/data/hvsecrets",
    );
  });
  ...
  it("rejects traversal and unsafe segments", () => {
    ...
    expect(() => normalizeVaultPath("secret//value")).toThrow(
      "Invalid Vault path segment",
    );
    expect(() => normalizeVaultPath("secret/value?x=1")).toThrow(
      "Invalid Vault path segment",
    );
  });
